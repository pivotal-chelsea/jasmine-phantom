...
var timeToWaitForSpecsInSeconds = 45;
function waitForSpecsToFinish(page, seconds, whenDone) {
  if (jasmineReportsFinished(page)) {
    ...
    if (seconds > 0) {
      ...
      console.log("Jasmine tests didn't finish in " + timeToWaitForSpecsInSeconds + " seconds... bailing out");
      phantom.exit(2);
    }
  }
}
...
page.open(system.args[1], function(status) {
  if (status == 'success') {
    waitForSpecsToFinish(page, timeToWaitForSpecsInSeconds, function() {
